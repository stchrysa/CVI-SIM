 ...
 */
public class ERreq<V,E> implements GraphGenerator<Node,Link> {
     ...
     */
	public ERreq(Factory<UndirectedGraph<Node,Link>> graphFactory,
	...
	public Graph<Node,Link> create() {
		...
		for (int i = 0; i < mNumVertices-1; i++) {
			...
			for (int j = i+1; j < mNumVertices; j++) {
                Node v_j = list.get(j);
                if (((v_i instanceof VirtualMachine) && (v_j instanceof RequestRouter)) || 
                		((v_i instanceof RequestRouter) && (v_j instanceof RequestRouter)) ){
